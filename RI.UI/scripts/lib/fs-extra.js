// Source: https://github.com/jprichardson/node-fs-extra/tree/v2.1.2
'use strict';
...
function copyFileFallback(srcStat, src, dest, opts) {
	...
	if (opts.preserveTimestamps)
		gracefulFs.futimesSync(fdw, srcStat.atime, srcStat.mtime);
		...
		if (destStat === notExist$1) {
			...
			gracefulFs.readlink(dest, (err, resolvedDest) => {
				...
				if (
					...
					isSrcSubdir$1(resolvedDest, resolvedSrc)
				) {
					return cb(
						new Error(
							`Cannot overwrite '${resolvedDest}' with '${resolvedSrc}'.`
...
function checkStats$1(src, dest, cb) {
	gracefulFs.stat(src, (err, srcStat) => {
		...
		gracefulFs.stat(dest, (err, destStat) => {
			if (err) {
				if (err.code === 'ENOENT')
					return cb(null, { srcStat, destStat: notExist$1 });
				return cb(err);
			}
